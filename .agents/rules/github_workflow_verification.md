...
- **Zero Tolerance for Workflow Failures**:
  - If any workflow job fails (e.g. `CI`, `Build, Package & Publish Extension`, `Lint`, `Store Publishing`), the task is considered **incomplete**.
...
[Code / Config Changes]
         │
         ▼
[Run Local Builds / Tests (npm run build:* / test)]
         │
         ▼
...
- If an automated bot or workflow pushed ref updates, run `git pull --rebase` to synchronize local tracking branches.
