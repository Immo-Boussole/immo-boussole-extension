import { t } from '../../i18n';
export function isHektor() {
    const host = window.location.hostname;
    const path = window.location.pathname;
    const search = window.location.search;
    return (host.includes('immoreve.fr') ||
...
export function isHektorDetailPage() {
    if (!isHektor())
        return false;
    const path = window.location.pathname;
    const search = window.location.search;
    return (document.querySelector('hektor-public-property-page') !== null ||
...
export function extractHektorDetailPage() {
    ...
    // Price
    const priceEl = document.querySelector('.properties-detail__price') ||
    ...
    imgEls.forEach((img) => {
        ...
        const src = img.getAttribute('data-splide-lazy') ||
...
export function injectHektorButtons(onAdd) {
    ...
    cardLinks.forEach((link) => {
        const card = link.closest('article, .item, .card, [class*="card"], [class*="item"]');
